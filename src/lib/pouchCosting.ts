// Pouch costing model for Contract-Packaging (Pouches / stand-up bags) quotes.
//
// WHAT THIS IS
//
// The pouch sibling of lib/blisterCosting.ts, built the same way: everything
...
// rules still lives in exactly one place. Only what is pouch-specific is
// defined here:
//
//   LINE SPEED    pouches/min (nameplate) × (1 − penalty%). The pouch machine
//                 has a rated PPM but real lines lose time to film splices,
//                 zipper feed jams and reel changes. The house penalty
...
//
//   FINISHED UNIT Pricing is per FINISHED UNIT, not per pouch. With
//                 secondary/retail packaging several pouches can go into one
//                 carton — pouchesPerUnit carries that. Line hours run on
//                 POUCHES (quantity × pouchesPerUnit); everything else runs
//                 on units.
//
...
// PouchCostingBoard imports everything from this module and cannot end up
...
/**
 * The house speed penalty. A pouch machine's nameplate PPM assumes the film
 ...
 */
export const DEFAULT_POUCH_SPEED_PENALTY_PCT = 20;

export type PouchLaborInputs = {
  ...
  /**
   * Pouches in one FINISHED UNIT. 1 for a bare pouch; with secondary/retail
  ...
  setup: LaborPhase;
  /** The pouch line itself. hours null = derive from the speed chain. */
  line: LaborPhase;
  printing: LaborPhase;
  packout: LaborPhase;
...
/**
 * The planning line speed in pouches per minute:
 *
 ...
 */
export function effectivePouchesPerMinute(
  labor: Pick<PouchLaborInputs, "pouchesPerMinute" | "speedPenaltyPct">,
): number | null {
  const ppm = num(labor.pouchesPerMinute);
  const pen = num(labor.speedPenaltyPct) ?? DEFAULT_POUCH_SPEED_PENALTY_PCT;
...
/**
 * Line time in hours. The line fills POUCHES, not finished units, so the
 * quantity is multiplied out first:
 *
 *   hours = (units × pouches per unit) ÷ (effective pouches/min × 60)
 */
export function pouchProductionHours(
  quantity: number | null,
  labor: PouchLaborInputs,
): number | null {
  const q = num(quantity);
  const ppu = num(labor.pouchesPerUnit) ?? 1;
  const eff = effectivePouchesPerMinute(labor);
 ...
 */
export function pouchLaborBreakdown(
  quantity: number | null,
  labor: PouchLaborInputs,
): LaborBreakdown | null {
  const q = num(quantity);
  ...
  const lineHours = num(labor.line.hours) ?? pouchProductionHours(q, labor);
  ...
  // Process order: set up, run the pouch line, and while it runs the hand
  ...
  const phases = [
    ...
    mk("Pouch Line", lineHours, labor.line.leaders, labor.line.operators),
    mk(
      "Printing",
      printingHours,
...
export function pouchLaborPerUnit(
  quantity: number | null,
  labor: PouchLaborInputs,
): number | null {
  return pouchLaborBreakdown(quantity, labor)?.perUnit ?? null;
}

/**
 * Top level — the same pipeline as computeBlisterCosting, with the pouch
 ...
 */
export function computePouchCosting(
  input: PouchCostingInputs,
): BottleCostingResult {
  const q = num(input.quantity);

  const mat = materialsPerUnit(input.bom);
  const prodHours = pouchProductionHours(q, input.labor);
  const lb = pouchLaborBreakdown(q, input.labor);
