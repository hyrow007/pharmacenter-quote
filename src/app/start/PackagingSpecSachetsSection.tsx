...
// Packaging Specification Form (Sachets) — inline questionnaire for
// contract-packaged sachets. There is no PandaDoc sachet form yet, so this is
// the pouch form (sections A–J) adapted: sachet size instead of bag size, no
// hanging hole or zipper, and lot / EXP always printed ON the line (sachets
// are never printed off the line). Fillable later — nothing here gates Save.
//
// Sachet film is roll stock charged by the impression, i.e. per sachet, so
// the costing board prices it per each (one impression per sachet).

import { useState, type CSSProperties } from "react";
import {
  blankPackagingSpecSachets,
  type PackagingSpecSachets,
} from "@/lib/workflows";

type SpecKey = Exclude<keyof PackagingSpecSachets, "formVersion">;

type Props = {
  spec: PackagingSpecSachets | undefined;
  // Mirrors setProduct's updater pattern: receives the current (or blank)
  // spec and must return the next one.
  onChange: (
    updater: (cur: PackagingSpecSachets) => PackagingSpecSachets,
  ) => void;
};
...
type Ctx = { s: PackagingSpecSachets; set: (k: SpecKey, v: string) => void };
...
export default function PackagingSpecSachetsSection({ spec, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const s = spec ?? blankPackagingSpecSachets();
      ...
      {open ? (
        <div style={{ padding: "0 14px 14px" }}>
          <p style={{ fontSize: 12, color: "var(--ink-3)", margin: "0 0 4px" }}>
            Adapted from the pouch Packaging Form (no PandaDoc sachet form yet).
            Nothing here blocks saving — answer what you know now and finish
            later.
          </p>
          ...
          <div style={groupTitle}>Sachet</div>
          <div style={rowStyle}>
            <Text ctx={ctx} k="sachetCount" label="Count (doses per sachet)" placeholder="e.g. 2" />
            <Supplied ctx={ctx} k="filmSuppliedBy" label="Sachet/film supplied by" />
            <Pick ctx={ctx} k="filmMaterial" otherK="filmMaterialOther" label="Film material" opts={OPTS.filmMaterial} />
          </div>
          <div style={rowStyle}>
            <Text ctx={ctx} k="bagWidth" label="Sachet width" placeholder={'e.g. 2"'} />
            <Text ctx={ctx} k="bagHeight" label="Sachet height" placeholder={'e.g. 3"'} />
          ...
          <div style={groupTitle}>Film printing (on the line)</div>
          <div style={rowStyle}>
          ...
          <div style={rowStyle}>
            ...
            {s.retailRequired === "yes" ? (
              <>
                ...
                <Text ctx={ctx} k="retailBagsPerPack" label="Sachets per pack" placeholder="e.g. 30" />
              </>
            ) : null}
          </div>
          {s.retailRequired === "yes" ? (
