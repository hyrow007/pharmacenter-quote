...
/**
 * Pouch costing board for Contract-Packaging (Pouches / stand-up bags)
 * workflows.
 *
 * Forked from BottleCostingBoard (v80). The math lives in
 * lib/pouchCosting.ts, which re-exports lib/bottleCosting.ts wholesale and
 * swaps in the pouch labour model: line speed = pouches/min (nameplate)
 * x (1 - penalty%), pricing per FINISHED UNIT (several pouches per
...
import {
  computePouchCosting,
  pouchLaborBreakdown,
  effectivePouchesPerMinute,
  DEFAULT_POUCH_SPEED_PENALTY_PCT,
  DEFAULT_LEADER_RATE,
  ...
  type CostStatus,
  type PackagingSlot,
  type PouchCostingInputs,
  type MarginMode,
  BREAKEVEN_OP_PROFIT_PER_RUN_DAY,
  BREAKEVEN_ASOF,
} from "@/lib/pouchCosting";
import {
  LeaseBreakdownTable,
...
function NumField({
  ...
  // 5,000-pouch film yield reads as five thousand at a glance where 5000
...
export type SavedState = {
  ...
  baseName?: string;
  /** Machine pouches per minute — the nameplate figure. */
  pouchesPerMinute: number | null;
  /** % knocked off the nameplate speed. Default 20 — see pouchCosting. */
  speedPenaltyPct: number | null;
  /**
   * Pouches in one FINISHED UNIT. 1 for a bare pouch; the carton count when
   ...
   * foot): the part's cost is per UOM, not per pouch. This says how many
   * POUCHES one UOM forms, so
   *
   *   qty per finished unit = pouches per unit / pouches per UOM
   *
   * and the ordinary qty x cost x waste arithmetic prices the film correctly.
   * PREMADE stand-up bags are the yield-1 case: one bag per pouch, priced
...
type SlotDef = {
  ...
  slot: PackagingSlot;
  label: string;
  /** Spec yes/no that decides presence. null = inherent to any pouch job. */
  presenceKey: string | null;
  /** Spec answer that decides who supplies it. */
  suppliedKey: string | null;
  /** Starting scrap %: pouch material 5, labels 10, boxes 2-3. */
  waste: number;
  /** Consumed once per POUCH (not per finished unit): the pouch material. */
  perPouch?: boolean;
};
...
const SLOTS: SlotDef[] = [
  ...
  // The pouch material — premade stand-up bags (yield 1, priced per each) or
 ...
 * Two pouch-specific exceptions below.
 */
function slotDefInSpec(
  ...
  if (
    ...
    return false;
  // The inner pack is the reverse of the bottle board's: the pouch form DOES
  ...
  if (!def.presenceKey) return true; // bulk, pouch material, master box
...
export function blankState(
  ...
  // Pouches per finished unit, straight off the packaging form. 1 when the
  // form is silent — a bare pouch IS the finished unit.
  ...
  // Doses per finished unit: the pouch count times the pouches that go into
...
function ComponentPicker({
  ...
  const suggested = useMemo(() => {
    ...
    const bits: string[] = [];
    if (slotKey === "film") {
      // Pouch spec: filmMaterial is the picklist (mylar-foil / kraft /
      // pet-pe / matte-bopp / other), filmMaterialOther the free text.
      bits.push(pick("filmMaterialOther", "filmMaterial") || "pouch");
          ...
          {rows.map((r) => (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, marginTop: 2 }}>
                ...
                {/* On the pouch-material row a UOM-unresolved part is still
...
export default function PouchCostingBoard({
  workflowId,
  quoteNumber,
  customerName,
  products,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // The key is the DEPARTMENT, not this calculator. Pouches, sachets,
    ...
    setSt((p) => ({
      ...
      bom: [
        ...p.bom,
        {
          ...
          // units it holds; the pouch material counts per pouch; everything
  ...
  /**
   * The planning line speed: pouches/min (nameplate) x (1 - penalty).
  ...
  const effBpm = useMemo(
    ...
    [st.pouchesPerMinute, st.speedPenaltyPct],
  );
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // pouchCosting keeps its historical meaning (first product) so every
      // existing reader still works, and products 2..n ride in
      // pouchCostingMore, index-aligned with state.products[1..].
      ...
      const res = await fetch(`/api/workflows/${workflowId}`, {
        ...
        body: JSON.stringify({
          state: {
            pouchCosting: bundles[0],
            pouchCostingMore: bundles.slice(1),
          },
        }),
      });
  ...
  useEffect(() => {
    ...
    document.title = [quoteNumber, productName || "Pouch Costing", "Costing"]
      .filter(Boolean)
      .join(" - ");
    ...
    >
      ...
      <div className="bc-card" style={shell}>
        ...
        <div style={{ ...metricGrid, rowGap: 24, paddingBottom: 22 }}>
          ...
          <ParamBlock label="Line speed (pouches / minute)">
          ...
          <ParamBlock label="Pouches / finished unit" nowrap>
            <NumField
              value={st.pouchesPerUnit}
              onChange={(v) => set("pouchesPerUnit", v)}
              placeholder="1"
            />
          </ParamBlock>
                ...
                <div>
                  {line.notUsed ? (
                    ...
                    <ComponentPicker
                      ...
                      spec={spec}
                      onPick={(opt) => {
                        // The pouch material is priced PER UOM (a roll, a
      ...
      <div className="bc-card bc-page" style={shell}>
        ...
        {lb === null ? (
          ...
          >
            ...
            <strong>No production estimate yet.</strong> Enter pouches per
          ...
          <>
            ...
            <div className="bc-sub" style={labSub}>
                    ...
                    {shownPhases.map((p) => (
                      <td key={p.label} style={labTd}>
                        {/* Every quantity-driven phase is READ-ONLY. Each is
                            ...
                            the line of the pouch speed chain, the hand
              ...
              >
                ...
                (units × pouches per unit) ÷ line speed; printing, packout,
