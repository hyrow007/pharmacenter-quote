...
export default function BlisterCostingBoard({
  ...
  const productName =
    products[activeBaseIdx]?.name ?? "Blistered product";
  ...
  const refreshFishbowlCostsNow = async () => {
    ...
    try {
      ...
      const res = refreshBomCosts(st.bom, rows, (line) => Boolean(slotKeyOf(line)?.perBlister));
    ...
    (async () => {
      ...
      setSt((p) => ({
        ...
        bom: p.bom.map((l) => {
          ...
          applied[l.id] = code;
          return refreshBomCosts([{ ...l, fpCode: code }], rows, (l) => Boolean(slotKeyOf(l)?.perBlister)).bom[0];
        }),
      }));
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // blisterCosting keeps its historical meaning (first product) so every
      // existing reader still works, and products 2..n ride in
      // blisterCostingMore, index-aligned with state.products[1..].
      ...
      const res = await fetch(`/api/workflows/${workflowId}`, {
        ...
        body: JSON.stringify({
          state: {
            blisterCosting: bundles[0],
            blisterCostingMore: bundles.slice(1),
          },
        }),
      });
