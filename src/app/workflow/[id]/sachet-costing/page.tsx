...
import SachetCostingBoard, {
  type BoardProduct,
  type SavedState,
} from "./SachetCostingBoard";

// /workflow/[id]/sachet-costing
//
// Builds the cost of ONE finished unit (a sachet, or the carton several
// sachets go into) from its bill of materials, the line and hand-station
// crews, and a share of overhead — a clone of the pouch board: sachet film
// is charged per impression (per sachet) and there is no off-line printing.
//
...
export default async function SachetCostingPage({ params }: Ctx) {
  ...
  // This board only makes sense for contract-packaged sachets. Anything else
  ...
  const isFinishedProduct =
    ...
    String(state.packagingType ?? "") === "sachets";
  if (!isFinishedProduct && (type !== "contract-packaging" || form !== "sachets")) {
  ...
  // sachetCosting keeps its historical meaning — the FIRST product's cost
  // build-up — so every reader of single-product workflows still works.
  // Products 2..n ride in sachetCostingMore, index-aligned.
  ...
  const products: BoardProduct[] = productRows.map((product, i) => {
    ...
    const initial =
      i === 0
        ? ((state.sachetCosting as SavedState | undefined) ?? null)
        : (more[i - 1] ?? null);
    ...
    <div className="app-shell">
      ...
      <main className="page">
        {/* Full 1240px shell, same reasoning as the bottle board: this IS the
            pricing calculator for CP-sachets, so it reads at the same width
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <h1 className="page-header__title" style={{ marginBottom: 6 }}>
              {isFinishedProduct
                ...
                : "Pricing Calculator · Sachets"}
            </h1>
            <p className="lede" style={{ marginTop: 4, marginBottom: 0 }}>
              Build the price of one finished unit from its bulk, sachet
