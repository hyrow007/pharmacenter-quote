...
/**
 * Sachet costing board for Contract-Packaging (Sachets) and Finished Product
 * (Sachets) workflows.
 *
 * Cloned from PouchCostingBoard (2026-09-21), with two sachet facts: the film
 * is roll stock charged per IMPRESSION (one per sachet, so it prices per
 * each), and sachets are never printed off the line (no printing station).
 * The pouch board was itself forked from BottleCostingBoard (v80). The math lives in
 * lib/sachetCosting.ts, which re-exports lib/bottleCosting.ts wholesale and
 * swaps in the sachet labour model: line speed = sachets/min (nameplate)
 * x (1 - penalty%), pricing per FINISHED UNIT (several sachets per
 * carton), and hand stations (printing / packout / cartoning / bundling)
 * derived from per-person speeds the way bottle kitting is.
 *
...
import {
  ...
  type CostStatus,
  type PackagingSlot,
  type SachetCostingInputs,
  type MarginMode,
  BREAKEVEN_OP_PROFIT_PER_RUN_DAY,
  BREAKEVEN_ASOF,
} from "@/lib/sachetCosting";
import {
  LeaseBreakdownTable,
...
function NumField({
  ...
  // 5,000-sachet film yield reads as five thousand at a glance where 5000
...
export type SavedState = {
  ...
  /**
   * Sachets in one FINISHED UNIT. 1 for a bare sachet; the carton count when
   * secondary packaging exists. Seeded from the spec's retailBagsPerPack.
  ...
  /**
   * FILM YIELD — the bridge between how Fishbowl prices the sachet material
   * and how the job consumes it. Roll stock is bought by the roll (or kg, or
   * foot): the part's cost is per UOM, not per sachet. This says how many
   * SACHETS one UOM forms, so
   *
   *   qty per finished unit = sachets per unit / sachets per UOM
   *
   * and the ordinary qty x cost x waste arithmetic prices the film correctly.
   * PREMADE stand-up bags are the yield-1 case: one bag per sachet, priced
   * per each. Null = not stated yet, which BLOCKS the film line rather than
   * multiplying a roll price by the sachet count and quoting nonsense.
   */
  /** DEPRECATED — read by nothing. Kept so quotes saved while the film
   *  yield input existed still load against this type. */
  filmSachetsPerUom: number | null;
  /**
  ...
  printingLeaders: number | null;
  printingOperators: number | null;
...
type SlotDef = {
   ...
   * so the Fishbowl category alone cannot tell the rows apart. */
  key: string;
  /** packaging_components.category the picker filters on. */
  slot: PackagingSlot;
  label: string;
  /** Spec yes/no that decides presence. null = inherent to any sachet job. */
  presenceKey: string | null;
  /** Spec answer that decides who supplies it. */
  suppliedKey: string | null;
  /** Starting scrap %: sachet material 5, labels 10, boxes 2-3. */
  waste: number;
  /** Consumed once per SACHET (not per finished unit): the sachet material. */
  perSachet?: boolean;
};
...
const SLOTS: SlotDef[] = [
  ...
  // The sachet material — premade stand-up bags (yield 1, priced per each) or
  // roll stock (yield = sachets one UOM of film forms). The Honey gummy sheet
  ...
  { key: "film", slot: "other", label: "Sachet film (per impression)", presenceKey: null, suppliedKey: "filmSuppliedBy", waste: 5, perSachet: true },
 ...
 * Two sachet-specific exceptions below.
 */
function slotDefInSpec(
  ...
  if (
    ...
    return false;
  // The inner pack is the reverse of the bottle board's: the sachet form DOES
  ...
  if (!def.presenceKey) return true; // bulk, sachet material, master box
...
export function blankState(
  ...
  // Sachets per finished unit, straight off the packaging form. 1 when the
  // form is silent — a bare sachet IS the finished unit.
  const bpuRaw = Number(spec?.retailBagsPerPack ?? "");
  ...
  // Doses per finished unit: the sachet count times the sachets that go into
  ...
  return {
    ...
    bom: SLOTS.filter((s) => slotDefInSpec(s, spec ?? null)).map((s, i) => ({
      ...
      fpCode: null,
      name: s.label,
      // Sachet film starts blank here and is filled at once by the qty
      // effect (sachets per unit). A master box is shared across the units
    ...
    cleaningLeaders: 0,
    cleaningOperators: 2,
    // The Honey gummy sheet: printing ran with 2, packout with 1, carton
    // printing and bundling with 2 each — hand stations, all operators.
    printingLeaders: 0,
    printingOperators: 2,
    packoutLeaders: 0,
    packoutOperators: 1,
...
function ComponentPicker({
  ...
  const suggested = useMemo(() => {
    ...
    const bits: string[] = [];
    if (slotKey === "film") {
      // Sachet spec: filmMaterial is the picklist (mylar-foil / kraft /
      // pet-pe / matte-bopp / other), filmMaterialOther the free text.
      bits.push(pick("filmMaterialOther", "filmMaterial") || "sachet");
          ...
          {rows.map((r) => (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, marginTop: 2 }}>
                ...
                {/* On the sachet-material row a UOM-unresolved part is still
      ...
      bundlingRequired:
        initial.bundlingRequired !== undefined
          ...
          : typeof initial.bundlingSpeed === "number" &&
            ...
            ? true
            : null,
      // Sachets are NEVER printed off the line — lot / EXP go on in the
      // machine — so the station is off whatever an old save says.
      printingOffLine: false,
...
export default function SachetCostingBoard({
  workflowId,
  quoteNumber,
  customerName,
  ...
  const productName =
    products[activeBaseIdx]?.name ?? "Sachet product";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // The key is the DEPARTMENT, not this calculator. Sachets, sachets,
        // blisters and kitting run on the same floor with the same equipment and
    ...
    setSt((p) => ({
      ...
      bom: [
        ...p.bom,
        {
          ...
          // units it holds; the sachet material counts per sachet; everything
            ...
            key === "inner_pack"
              ...
              : key === "master_box"
                ? p.bottlesPerMasterBox && p.bottlesPerMasterBox > 0
                  ...
                  : null
                : meta?.perSachet
                  ? p.sachetsPerUnit && p.sachetsPerUnit > 0
                    ? p.sachetsPerUnit
                    : 1
                  : 1,
          costPerUnit: null,
  ...
  /**
   * The planning line speed: sachets/min (nameplate) x (1 - penalty).
  ...
  useEffect(() => {
    ...
    setSt((p) => ({
      ...
      bom: p.bom.map((l) => {
        ...
        // Sachet film is roll stock, but it is CHARGED per impression — one
  ...
  const inputs: SachetCostingInputs = useMemo(
    () => ({
      quantity: qty,
      ...
      labor: {
        ...
        // Never an off-line printing station on sachets.
        printingSpeed: null,
  ...
  const r = useMemo(() => computeSachetCosting(inputs), [inputs]);
   ...
   */
  const lb = useMemo(
    () => sachetLaborBreakdown(qty, inputs.labor),
    [qty, inputs.labor],
  );
  ...
  const issueQuote = useCallback(() => {
    ...
    const html = buildQuoteHtml({
      ...
      quoteKind: finishedProduct
        ...
        : "Contract Packaging Quote (Sachets)",
    });
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // sachetCosting keeps its historical meaning (first product) so every
      // existing reader still works, and products 2..n ride in
      // sachetCostingMore, index-aligned with state.products[1..].
      ...
      const res = await fetch(`/api/workflows/${workflowId}`, {
        ...
        body: JSON.stringify({
          state: {
            sachetCosting: bundles[0],
            sachetCostingMore: bundles.slice(1),
          },
        }),
      });
  ...
  useEffect(() => {
    ...
    document.title = [quoteNumber, productName || "Sachet Costing", "Costing"]
      .filter(Boolean)
      .join(" - ");
    ...
    >
      ...
      <div className="bc-card" style={shell}>
        ...
        <div style={{ ...metricGrid, rowGap: 24, paddingBottom: 22 }}>
          ...
          <ParamBlock label="Sachets / minute" nowrap>
            <NumField
              value={st.sachetsPerMinute}
              onChange={(v) => set("sachetsPerMinute", v)}
              placeholder="required"
            />
          </ParamBlock>
          {/* The nameplate speed is a promise the floor never keeps — film
              splices, zipper feed jams, reel changes. 20% off is the house
              planning figure; editable because a proven format earns some
              back (set 0 to run the nameplate flat, like the Honey sheet). */}
          ...
          {/* Per PERSON, unlike the line speed. The hand stations scale with
              ...
              that station. */}
          {/* No `nowrap` on these four: the labels are the longest on the
                ...
                <div>
                  {line.notUsed ? (
                    ...
                    <ComponentPicker
                      ...
                      spec={spec}
                      onPick={(opt) => {
                        // The sachet material is priced PER UOM (a roll, a
      ...
      <div className="bc-card bc-page" style={shell}>
        ...
        {lb === null ? (
          ...
          >
            ...
            <strong>No production estimate yet.</strong> Enter sachets per
            minute in Considerations above. The hours tables appear once
            there is a line speed.
          </div>
        ) : (
          <>
            {/* ---- Hours ----
                    ...
                    {shownPhases.map((p) => (
                      <td key={p.label} style={labTd}>
                        {/* Every quantity-driven phase is READ-ONLY. Each is
                            ...
                            the line of the sachet speed chain, the hand
